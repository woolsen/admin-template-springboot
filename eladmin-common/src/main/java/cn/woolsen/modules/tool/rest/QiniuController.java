 ...
 */
package cn.woolsen.modules.tool.rest;

import cn.woolsen.annotation.Log;
import cn.woolsen.base.PageDTO;
import cn.woolsen.modules.tool.domain.QiniuConfig;
import cn.woolsen.modules.tool.domain.QiniuContent;
import cn.woolsen.modules.tool.domain.dto.QiniuQueryCriteria;
import cn.woolsen.modules.tool.domain.dto.QiniuResult;
import cn.woolsen.modules.tool.service.QiNiuService;
...
public class QiniuController {
    ...
    @ApiOperation("查询文件")
    @GetMapping
    public ResponseEntity<PageDTO<QiniuContent>> queryQiNiu(QiniuQueryCriteria criteria, Pageable pageable) {
    ...
    @ApiOperation("上传文件")
    @PostMapping
    public QiniuResult uploadQiNiu(@RequestParam MultipartFile file) {
        QiniuContent qiniuContent = qiNiuService.upload(file, qiNiuService.find());
        QiniuResult result = new QiniuResult();
        result.setId(qiniuContent.getId());
        result.setErrno(0);
        result.setData(new String[]{qiniuContent.getUrl()});
        return result;
    }
